...
function Marquee() {
  return <div className="overflow-hidden border-y border-[#f3eee4]/15 py-4 font-mono-ui text-[10px] uppercase tracking-[.24em] text-[#f3eee4]/60"><div className="marquee-track flex w-max"><span className="flex items-center gap-8 pr-8">Thoughtful materials <i className="h-1.5 w-1.5 rounded-full bg-[#c96c48]" /> Made to be lived in <i className="h-1.5 w-1.5 rounded-full bg-[#b99a63]" /> See it in your room <i className="h-1.5 w-1.5 rounded-full bg-[#c96c48]" /></span><span className="flex items-center gap-8 pr-8">Thoughtful materials <i className="h-1.5 w-1.5 rounded-full bg-[#c96c48]" /> Made to be lived in <i className="h-1.5 w-1.5 rounded-full bg-[#b99a63]" /> See it in your room <i className="h-1.5 w-1.5 rounded-full bg-[#c96c48]" /></span></div></div>;
}
  ...
  return (
    <main className="min-h-screen bg-[#171512] px-5 pb-28 pt-32 md:px-10 md:pt-44">
      <div className="mx-auto max-w-[1440px]">
        ...
        {filtered.length > 0 ? <div className="mt-10 grid gap-x-6 gap-y-14 sm:grid-cols-2 lg:grid-cols-4">{filtered.map((product) => <ProductCard key={product.id} product={product} liked={liked.includes(product.id)} compared={compared.includes(product.id)} onLike={() => onLike(product.id)} onCompare={() => onCompare(product.id)} onAdd={() => onAdd(product)} />)}</div> : <div className="py-28 text-center"><Sparkles className="mx-auto text-[#bd8250]" /><h2 className="mt-5 font-display text-5xl">Nothing in that register.</h2><p className="mt-3 text-sm text-[#f3eee4]/55">Try another room or style.</p><button className="mt-7 border-b border-[#bd8250] pb-1 text-xs uppercase tracking-[.15em]" onClick={() => { setFilter('All pieces'); setMaterialFilter('All materials'); setSort('Featured'); }} data-testid="button-reset-filters">Reset filters</button></div>}
      </div>
    </main>
  );
}
...
function AdminPage({ user }: { user: SessionUser | null }) {
  ...
  if (!firebaseEnabled) {
    return <main className="min-h-screen bg-[#171512] px-5 pb-28 pt-32 md:px-10 md:pt-44"><div className="mx-auto max-w-[900px]"><p className="eyebrow">FurniVision / Admin</p><h1 className="mt-5 font-display text-8xl leading-[.76] tracking-[-.06em] md:text-[10rem]">Admin<br /><em>offline.</em></h1><p className="mt-8 max-w-lg text-sm leading-6 text-[#f3eee4]/65">Firebase is not configured in this environment, so the admin panel cannot verify a staff account or write catalog records. Add the variables from .env.example, then refresh this page.</p><Link href="/account" className="mt-8 inline-flex rounded-full bg-[#0b0b0a] px-6 py-4 text-xs uppercase tracking-[.15em] text-[#f3eee4]" data-testid="link-admin-account">Open account setup</Link></div></main>;
  }
  if (!user) {
    return <main className="min-h-screen bg-[#171512] px-5 pb-28 pt-32 md:px-10 md:pt-44"><div className="mx-auto max-w-[900px]"><p className="eyebrow">FurniVision / Admin</p><h1 className="mt-5 font-display text-8xl leading-[.76] tracking-[-.06em] md:text-[10rem]">Sign in<br /><em>first.</em></h1><p className="mt-8 max-w-lg text-sm leading-6 text-[#f3eee4]/65">The catalog workspace is restricted to authenticated staff accounts.</p><Link href="/account" className="mt-8 inline-flex rounded-full bg-[#0b0b0a] px-6 py-4 text-xs uppercase tracking-[.15em] text-[#f3eee4]" data-testid="link-admin-sign-in">Go to sign in</Link></div></main>;
  }
  ...
  if (!allowed) return <main className="min-h-screen bg-[#0b0b0a] px-5 pb-28 pt-32 text-[#f3eee4] md:px-10 md:pt-44"><div className="mx-auto max-w-[900px]"><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[#b99a63]">FurniVision / Admin</p><h1 className="mt-5 font-display text-8xl leading-[.76] tracking-[-.06em] md:text-[10rem]">Access<br /><em>denied.</em></h1><p className="mt-8 max-w-lg text-sm leading-6 text-[#f3eee4]/60">Your account is signed in, but it does not have the Firebase <code className="text-[#b99a63]">admin</code> custom claim required to manage the catalog.</p><Link href="/account" className="mt-8 inline-flex rounded-full bg-[#b99a63] px-6 py-4 text-xs uppercase tracking-[.15em] text-[#f3eee4]" data-testid="link-admin-access-account">Return to account</Link></div></main>;
    ...
    <main className="min-h-screen bg-[#b99a63] px-5 pb-28 pt-32 md:px-10 md:pt-44">
      <div className="mx-auto grid max-w-[1180px] gap-12 lg:grid-cols-[1fr_420px] lg:items-end">
        ...
        <div className="rounded-[1.5rem] bg-[#171512] p-7 md:p-8">
          ...
          {!firebaseEnabled && <p className="mt-5 rounded-xl bg-[#e7e0d6] p-3 text-xs leading-5 text-[#f3eee4]/65">Browsing works in demo mode. Add the Firebase variables from .env.example to enable accounts and cloud sync.</p>}

...
import { assignOrder, createWorkerAccount, isCurrentUserWorker, loadAllOrders, loadAssignedOrders, loadCurrentWorker, loadWorkers, saveCustomerOrder, updateAssignedOrderStatus, type CustomerDetails, type StoreOrder, type WorkerProfile } from './lib/workers';
...
function App() {
  ...
  useEffect(() => subscribeToAuth((nextUser) => {
    ...
    setUser(nextSession);
    if (nextUser) void isCurrentUserWorker().then((worker) => { if (worker) setLocation('/worker'); }).catch(() => undefined);
  }), []);
  useEffect(() => {
    if (!user) {

...
function DesignPage({ onSaveRoom }: { onSaveRoom: (room: string, selected: string, photo: string | null) => void }) {
  ...
  useEffect(() => subscribeToAuth((nextUser) => {
    if (!nextUser) { setAuthUser(null); return; }
    void ensureFirstUserAdmin(nextUser).catch(() => undefined).finally(() => {
      setAuthUser({ uid: nextUser.uid, email: nextUser.email, displayName: nextUser.displayName });
    });
  }), []);
...
function AdminSignInPage() {
  ...
  const routeStaffUser = async (user: Pick<SessionUser, 'uid'>) => {
    ...
    if (await isCurrentUserAdmin()) setLocation('/admin');
      ...
      await signOutUser();
      setMessage('This account is not an admin or an active worker.');
    }
  };
...
function WorkerPage({ user }: { user: SessionUser | null }) {
  ...
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      const profile = await loadCurrentWorker();
      if (!profile?.active) {
        setWorker(null);
        return;
      }
      setWorker(profile);
      setOrders(await loadAssignedOrders(profile.uid));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'We could not load your assigned orders.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateStatus = async (orderId: string, status: string) => {
    ...
    setMessage('');
    try {
      await updateAssignedOrderStatus(orderId, status);
  ...
  if (!firebaseEnabled) return <main className="min-h-screen bg-[#171512] px-5 pb-28 pt-32 text-[#f3eee4]"><div className="mx-auto max-w-[900px]"><p className="eyebrow">FurniVision / Worker panel</p><h1 className="mt-5 font-display text-8xl leading-[.76]">Panel<br /><em>offline.</em></h1><p className="mt-8 max-w-lg text-sm leading-6 text-[#f3eee4]/65">Firebase is not configured, so worker accounts and assigned orders are unavailable.</p></div></main>;
  if (!user) return <AdminSignInPage />;
  if (loading) return <main className="flex min-h-screen items-center justify-center bg-[#b99a63] px-5 pt-24"><p className="font-mono-ui text-[10px] uppercase tracking-[.16em]">Loading your route sheet…</p></main>;
  if (!worker) return <main className="min-h-screen bg-[#0b0b0a] px-5 pb-28 pt-32 text-[#f3eee4] md:px-10 md:pt-44"><div className="mx-auto max-w-[900px]"><p className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[#b99a63]">FurniVision / Worker panel</p><h1 className="mt-5 font-display text-8xl leading-[.76]">Access<br /><em>denied.</em></h1><p className="mt-8 max-w-lg text-sm leading-6 text-[#f3eee4]/60">This account is not an active worker. Ask the storefront owner to create or reactivate your worker account.</p><button onClick={async () => { await signOutUser(); setLocation('/'); }} className="mt-8 rounded-full bg-[#b99a63] px-6 py-4 text-xs uppercase tracking-[.15em] text-[#0b0b0a]">Sign out</button></div></main>;
...
function AdminPage({ user }: { user: SessionUser | null }) {
  ...
  const [activePanel, setActivePanel] = useState<'overview' | 'products' | 'homepage' | 'settings' | 'pages' | 'orders' | 'workers'>('overview');
  const [orders, setOrders] = useState<Order[]>([]);
  const [workers, setWorkers] = useState<WorkerProfile[]>([]);
  const [workerDraft, setWorkerDraft] = useState({ name: '', email: '', password: '' });
  const [selectedWorkerByOrder, setSelectedWorkerByOrder] = useState<Record<string, string>>({});
  ...
  const createWorker = async (event: FormEvent) => {
    ...
    setSaving(true); setMessage('');
    try {
      const worker = await createWorkerAccount(workerDraft.name, workerDraft.email, workerDraft.password);
      setWorkers((current) => [worker, ...current]);
      setWorkerDraft({ name: '', email: '', password: '' });
      setMessage(`Worker account created for ${worker.email}. Share the password securely; it is not stored in this panel.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Could not create the worker account.');
    } finally { setSaving(false); }
  };
  const saveAssignment = async (order: Order) => {
    const workerId = selectedWorkerByOrder[order.id] ?? order.assignedWorkerId ?? '';
    const worker = workers.find((item) => item.uid === workerId) || null;
    setSaving(true); setMessage('');
    try {
      await assignOrder(order.id, worker);
      setOrders((current) => current.map((item) => item.id === order.id ? { ...item, assignedWorkerId: worker?.uid, assignedWorkerName: worker?.name, status: worker ? 'Assigned' : 'New', updatedAt: new Date().toISOString() } : item));
      setMessage(worker ? `Order ${order.id} assigned to ${worker.name}.` : `Order ${order.id} is back in the unassigned queue.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Could not assign this order.');
    } finally { setSaving(false); }
  };
  const toggleWorker = async (worker: WorkerProfile) => {
    const nextActive = !worker.active;
    if (!nextActive && !window.confirm(`Deactivate ${worker.name}'s login? They will lose access immediately.`)) return;
    setSaving(true); setMessage('');
    try {
      await updateWorkerActive(worker.uid, nextActive);
      setWorkers((current) => current.map((item) => item.uid === worker.uid ? { ...item, active: nextActive, updatedAt: new Date().toISOString() } : item));
      setMessage(nextActive ? `${worker.name}'s account is active again.` : `${worker.name}'s account was deactivated.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Could not update this worker.');
    } finally { setSaving(false); }
  };
  ...
  const storefrontPages = [
    ...
    { label: 'Checkout', path: '/checkout', description: 'Cart and checkout experience.' },
    { label: 'Inspiration', path: '/inspiration', description: 'Editorial inspiration and room stories.' },
    { label: 'Visualize a room', path: '/design', description: 'The room visualizer experience.' },
    { label: 'Account', path: '/account', description: 'Sign-in, account, and order history.' },
  ];
  const adminNavigation = [
    { id: 'overview' as const, label: 'Overview', icon: LayoutDashboard, group: 'Workspace' },
    { id: 'orders' as const, label: 'Orders', icon: ClipboardList, count: orders.length, group: 'Operations' },
    { id: 'workers' as const, label: 'Workers', icon: UsersRound, count: workers.length, group: 'Operations' },
    { id: 'products' as const, label: 'Products', icon: Package, group: 'Storefront' },
    { id: 'homepage' as const, label: 'Homepage & media', icon: Image, group: 'Storefront' },
    { id: 'pages' as const, label: 'Live pages', icon: Store, group: 'Storefront' },
    { id: 'settings' as const, label: 'Settings', icon: Settings2, group: 'System' },
  ];
  const activeNavigation = adminNavigation.find((item) => item.id === activePanel) || adminNavigation[0];

  return <main className="admin-shell min-h-screen bg-[#11110f] text-[#f3eee4]"><div className="mx-auto flex min-h-screen max-w-[1600px] flex-col lg:flex-row"><aside className="admin-sidebar border-b border-[#f3eee4]/10 bg-[#171512] px-5 py-5 lg:sticky lg:top-0 lg:h-screen lg:w-[248px] lg:shrink-0 lg:border-b-0 lg:border-r lg:px-4 lg:py-6"><div className="flex items-center lg:block"><Link href="/" className="inline-flex items-center gap-3 px-2" data-testid="link-admin-logo"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#b99a63] text-[#11110f]"><Box size={18} strokeWidth={1.8} /></span><span><span className="block font-display text-2xl leading-none">FurniVision</span><span className="mt-1 block font-mono-ui text-[8px] uppercase tracking-[.18em] text-[#f3eee4]/40">Admin studio</span></span></Link></div><nav className="mt-8 hidden lg:block" aria-label="Admin sections">{['Workspace', 'Operations', 'Storefront', 'System'].map((group) => <div key={group} className="mb-7"><p className="px-3 font-mono-ui text-[9px] uppercase tracking-[.18em] text-[#f3eee4]/30">{group}</p><div className="mt-2 grid gap-1">{adminNavigation.filter((item) => item.group === group).map((item) => { const Icon = item.icon; const active = activePanel === item.id; return <button key={item.id} onClick={() => setActivePanel(item.id)} className={`admin-nav-item flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition-colors ${active ? 'bg-[#b99a63] text-[#11110f]' : 'text-[#f3eee4]/55 hover:bg-[#f3eee4]/[.06] hover:text-[#f3eee4]'}`} data-testid={`button-admin-${item.id}`}><Icon size={16} strokeWidth={1.8} /><span className="flex-1">{item.label}</span>{item.count !== undefined && <span className={`font-mono-ui text-[9px] ${active ? 'text-[#11110f]/60' : 'text-[#f3eee4]/30'}`}>{item.count}</span>}</button>; })}</div></div>)}</nav><div className="mt-8 hidden rounded-2xl border border-[#f3eee4]/10 bg-[#1d1b18] p-4 lg:block"><div className="flex items-center gap-2 text-xs text-[#b99a63]"><span className="h-2 w-2 rounded-full bg-[#b99a63]" /> Firebase connected</div><p className="mt-2 text-[11px] leading-5 text-[#f3eee4]/40">Your content and delivery operations are live.</p></div><button onClick={async () => { await signOutUser(); window.location.href = '/'; }} className="mt-8 hidden w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm text-[#f3eee4]/45 transition-colors hover:bg-[#f3eee4]/[.06] hover:text-[#f3eee4] lg:flex" data-testid="button-admin-sign-out"><LogOut size={16} /> Sign out</button></aside><div className="min-w-0 flex-1"><header className="border-b border-[#f3eee4]/10 bg-[#171512]/90 px-5 py-5 backdrop-blur-md md:px-8 lg:px-10"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-center"><div><div className="flex items-center gap-2 font-mono-ui text-[9px] uppercase tracking-[.16em] text-[#f3eee4]/35"><span>Admin studio</span><span>/</span><span className="text-[#b99a63]">{activeNavigation.label}</span></div><h1 className="mt-3 font-display text-4xl leading-none md:text-5xl">{activeNavigation.label}</h1></div><div className="flex items-center gap-2"><Link href="/" target="_blank" rel="noreferrer" className="hidden items-center gap-2 rounded-xl border border-[#f3eee4]/10 px-4 py-2.5 text-xs text-[#f3eee4]/60 transition-colors hover:border-[#b99a63]/50 hover:text-[#b99a63] sm:inline-flex" data-testid="link-admin-storefront"><Store size={14} /> View storefront <ArrowUpRight size={13} /></Link><button onClick={() => void window.location.reload()} className="rounded-xl border border-[#f3eee4]/10 p-2.5 text-[#f3eee4]/55 transition-colors hover:border-[#b99a63]/50 hover:text-[#b99a63]" aria-label="Refresh admin workspace" data-testid="button-admin-refresh"><RefreshCw size={15} /></button><div className="hidden h-9 w-px bg-[#f3eee4]/10 sm:block" /><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#b99a63] text-xs font-semibold text-[#11110f]">{(user?.email || 'A').slice(0, 1).toUpperCase()}</div></div></div><div className="mt-5 flex gap-2 overflow-x-auto pb-1 lg:hidden">{adminNavigation.map((item) => { const Icon = item.icon; const active = activePanel === item.id; return <button key={item.id} onClick={() => setActivePanel(item.id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-xs ${active ? 'bg-[#b99a63] text-[#11110f]' : 'border border-[#f3eee4]/10 text-[#f3eee4]/55'}`} data-testid={`button-admin-mobile-${item.id}`}><Icon size={14} />{item.label}{item.count !== undefined && <span className="opacity-60">· {item.count}</span>}</button>; })}</div></header><div className="px-5 pb-24 pt-6 md:px-8 lg:px-10 lg:pt-8">
     {message && <div className="mb-6 rounded-xl border border-[#b99a63]/30 bg-[#b99a63]/10 px-4 py-3 text-sm text-[#b99a63]" role="status">{message}</div>}
    {activePanel === 'pages' && <section className="mt-10"><div className="flex flex-col justify-between gap-5 border-b border-[#f3eee4]/15 pb-6 md:flex-row md:items-end"><div><p className="eyebrow">Storefront navigator</p><h2 className="mt-3 font-display text-5xl">Every room,<br /><em>one click away.</em></h2><p className="mt-4 max-w-xl text-sm leading-6 text-[#f3eee4]/60">Open the live customer experience from your workspace. Each preview opens in a new tab so you can keep your admin session available.</p></div><span className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#b99a63]">{storefrontPages.length} pages</span></div><div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{storefrontPages.map((page) => <article key={page.path} className="flex min-h-48 flex-col justify-between rounded-[1.25rem] border border-[#f3eee4]/15 bg-[#1d1b18] p-5"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#b99a63]">Preview</p><h3 className="mt-4 font-display text-3xl">{page.label}</h3><p className="mt-2 text-sm leading-5 text-[#f3eee4]/50">{page.description}</p></div><Link href={page.path} target="_blank" rel="noreferrer" className="mt-6 inline-flex w-fit items-center gap-2 border-b border-[#b99a63] pb-1 text-xs uppercase tracking-[.14em] text-[#f3eee4]" data-testid={'link-admin-page-' + page.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}>Open page <ArrowUpRight size={14} /></Link></article>)}</div></section>}
     {activePanel === 'overview' && <section className="mt-10"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><article className="rounded-2xl bg-[#24231f] p-5"><p className="eyebrow">Orders</p><p className="mt-5 font-display text-5xl">{orders.length}</p><p className="mt-2 text-xs text-[#f3eee4]/55">customer orders in the queue</p></article><article className="rounded-2xl bg-[#24231f] p-5"><p className="eyebrow">Workers</p><p className="mt-5 font-display text-5xl">{workers.length}</p><p className="mt-2 text-xs text-[#f3eee4]/55">delivery accounts</p></article><article className="rounded-2xl bg-[#24231f] p-5"><p className="eyebrow">Catalog</p><p className="mt-5 font-display text-5xl">{catalog.length}</p><p className="mt-2 text-xs text-[#f3eee4]/55">published product records</p></article><article className="rounded-2xl bg-[#b99a63] p-5 text-[#0b0b0a]"><p className="eyebrow">Next move</p><p className="mt-5 font-display text-3xl leading-[.9]">Assign the<br />next route.</p><button onClick={() => setActivePanel('orders')} className="mt-5 border-b border-[#0b0b0a] pb-1 text-xs uppercase tracking-[.14em]" data-testid="button-admin-start-orders">Open orders</button></article></div><div className="mt-10 grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><section className="rounded-[1.5rem] border border-[#f3eee4]/15 bg-[#1d1b18] p-6 md:p-8"><p className="eyebrow">Workspace map</p><div className="mt-6 divide-y divide-[#f3eee4]/10"><button onClick={() => setActivePanel('orders')} className="flex w-full items-center justify-between py-4 text-left"><span><strong className="font-display text-3xl">Orders</strong><span className="mt-1 block text-xs text-[#f3eee4]/50">Customer details, delivery status, worker assignment</span></span><ArrowRight size={17} /></button><button onClick={() => setActivePanel('workers')} className="flex w-full items-center justify-between py-4 text-left"><span><strong className="font-display text-3xl">Workers</strong><span className="mt-1 block text-xs text-[#f3eee4]/50">Create private delivery accounts</span></span><ArrowRight size={17} /></button><button onClick={() => setActivePanel('products')} className="flex w-full items-center justify-between py-4 text-left"><span><strong className="font-display text-3xl">Products</strong><span className="mt-1 block text-xs text-[#f3eee4]/50">Photos, pricing, inventory, descriptions</span></span><ArrowRight size={17} /></button></div></section><section className="rounded-[1.5rem] bg-[#24231f] p-6 md:p-8"><p className="eyebrow">Private operations</p><p className="mt-6 text-xl leading-relaxed text-[#f3eee4]/75">Workers see only the orders assigned to their Firebase user ID. The owner can view every order, assign routes, and change worker accounts without exposing the operations panel to customers.</p><div className="mt-8 flex items-center gap-3 text-xs text-[#f3eee4]/55"><Check size={17} className="text-[#b99a63]" /> Customer delivery details are restricted to assigned workers</div><div className="mt-4 flex items-center gap-3 text-xs text-[#f3eee4]/55"><Check size={17} className="text-[#b99a63]" /> Worker status updates cannot reassign orders</div></section></div></section>}
     {activePanel === 'orders' && <section className="mt-10"><div className="flex flex-col justify-between gap-5 border-b border-[#f3eee4]/15 pb-6 md:flex-row md:items-end"><div><p className="eyebrow">Order desk</p><h2 className="mt-3 font-display text-5xl">Put every<br /><em>delivery in motion.</em></h2></div><span className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#b99a63]">{orders.length} orders</span></div>{orders.length === 0 ? <div className="mt-8 rounded-[1.25rem] border border-[#f3eee4]/15 bg-[#1d1b18] p-8 text-sm text-[#f3eee4]/60">No customer orders have been placed yet.</div> : <div className="mt-8 grid gap-5">{orders.map((order) => <article key={order.id} className="rounded-[1.25rem] border border-[#f3eee4]/15 bg-[#1d1b18] p-5 md:p-7" data-testid={`admin-order-${order.id}`}><div className="flex flex-col justify-between gap-4 border-b border-[#f3eee4]/15 pb-5 md:flex-row md:items-start"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.15em] text-[#b99a63]">Order / {order.id}</p><h3 className="mt-2 font-display text-3xl">{order.customer.firstName} {order.customer.lastName}</h3><p className="mt-1 text-sm text-[#f3eee4]/55">{order.customer.email} · {order.customer.address}, {order.customer.city} · {order.customer.deliveryWindow}</p></div><div className="text-left md:text-right"><p className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#bd8250]">{order.status}</p><p className="mt-2 font-display text-2xl">{formatPrice(order.total)}</p></div></div><div className="mt-5 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="eyebrow">Items</p><p className="mt-2 text-sm text-[#f3eee4]/70">{order.items.map((item, index) => `${item.name} · ${item.material}`).join(' / ')}</p></div><div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto"><select value={selectedWorkerByOrder[order.id] ?? order.assignedWorkerId ?? ''} onChange={(event) => setSelectedWorkerByOrder((current) => ({ ...current, [order.id]: event.target.value }))} className="min-w-56 rounded-xl border border-[#f3eee4]/15 bg-[#24231f] px-4 py-3 text-sm text-[#f3eee4] outline-none focus:border-[#b99a63]" data-testid={`select-order-worker-${order.id}`}><option value="">Unassigned</option>{workers.filter((worker) => worker.active).map((worker) => <option value={worker.uid} key={worker.uid}>{worker.name} · {worker.email}</option>)}</select><button disabled={saving} onClick={() => void saveAssignment(order)} className="rounded-full bg-[#b99a63] px-5 py-3 text-xs uppercase tracking-[.14em] text-[#0b0b0a] disabled:opacity-50" data-testid={`button-assign-order-${order.id}`}>{saving ? 'Saving…' : 'Save assignment'}</button></div></div></article>)}</div>}</section>}
      {activePanel === 'workers' && <section className="mt-10 grid gap-8 lg:grid-cols-[.9fr_1.1fr]"><form onSubmit={createWorker} className="h-fit rounded-[1.5rem] bg-[#b99a63] p-6 text-[#0b0b0a] md:p-8"><p className="eyebrow">New worker</p><h2 className="mt-4 font-display text-5xl leading-[.88]">Give someone<br />a route.</h2><p className="mt-5 text-sm leading-6 text-[#0b0b0a]/65">Create the Firebase login here, then share the password with the worker through a secure channel. Passwords are never displayed again.</p><div className="mt-7 grid gap-4"><label className="text-xs uppercase tracking-[.13em]">Worker name<input required value={workerDraft.name} onChange={(event) => setWorkerDraft({ ...workerDraft, name: event.target.value })} className={fieldClass} autoComplete="name" data-testid="input-worker-name" /></label><label className="text-xs uppercase tracking-[.13em]">Worker email<input required type="email" value={workerDraft.email} onChange={(event) => setWorkerDraft({ ...workerDraft, email: event.target.value })} className={fieldClass} autoComplete="email" data-testid="input-worker-email" /></label><label className="text-xs uppercase tracking-[.13em]">Temporary password<input required minLength={6} type="password" value={workerDraft.password} onChange={(event) => setWorkerDraft({ ...workerDraft, password: event.target.value })} className={fieldClass} autoComplete="new-password" data-testid="input-worker-password" /></label><button disabled={saving} className="rounded-full bg-[#0b0b0a] py-4 text-xs uppercase tracking-[.15em] text-[#f3eee4] disabled:opacity-50" data-testid="button-create-worker">{saving ? 'Creating account…' : 'Create worker account'}</button></div></form><section className="rounded-[1.5rem] border border-[#f3eee4]/15 bg-[#1d1b18] p-6 md:p-8"><div className="flex items-end justify-between gap-4 border-b border-[#f3eee4]/15 pb-5"><div><p className="eyebrow">Your team</p><h2 className="mt-3 font-display text-5xl">Delivery<br /><em>accounts.</em></h2></div><span className="font-mono-ui text-[10px] uppercase tracking-[.14em] text-[#b99a63]">{workers.length} workers</span></div>{workers.length === 0 ? <p className="py-12 text-sm text-[#f3eee4]/55">No worker accounts yet. Create the first one on the left.</p> : <div className="mt-4 divide-y divide-[#f3eee4]/10">{workers.map((worker) => <article key={worker.uid} className="flex flex-col justify-between gap-4 py-5 sm:flex-row sm:items-center"><div><p className="font-display text-3xl">{worker.name}</p><p className="mt-1 text-sm text-[#f3eee4]/55">{worker.email}</p><p className="mt-2 text-xs text-[#f3eee4]/45">{orders.filter((order) => order.assignedWorkerId === worker.uid).length} assigned orders</p></div><div className="flex items-center gap-3"><span className={`w-fit rounded-full border px-3 py-1.5 font-mono-ui text-[9px] uppercase tracking-[.13em] ${worker.active ? 'border-[#b99a63]/40 text-[#b99a63]' : 'border-[#bd8250]/40 text-[#bd8250]'}`}>{worker.active ? 'Active' : 'Inactive'}</span><button type="button" disabled={saving} onClick={() => void toggleWorker(worker)} className="rounded-full border border-[#f3eee4]/20 px-3 py-2 text-[10px] uppercase tracking-[.12em] disabled:opacity-50" data-testid={`button-toggle-worker-${worker.uid}`}>{worker.active ? 'Deactivate' : 'Reactivate'}</button></div></article>)}</div>}</section></section>}
...
function AccountPage({ user, orders }: { user: SessionUser | null; orders: Order[] }) {
  ...
  useEffect(() => {
    let active = true;
    if (!user) { setIsAdmin(false); setIsWorker(false); return () => { active = false; }; }
    void Promise.all([isCurrentUserAdmin().catch(() => false), isCurrentUserWorker().catch(() => false)]).then(([admin, worker]) => {
      if (active) { setIsAdmin(admin); setIsWorker(worker); }
    }).catch(() => { if (active) { setIsAdmin(false); setIsWorker(false); } });
    return () => { active = false; };
  }, [user?.uid]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (result.user) {
      try {
        if (await isCurrentUserWorker()) setLocation('/worker');
        else {
          await ensureFirstUserAdmin(result.user);
          setLocation('/');
        }
      } catch { setMessage('Your account was created, but we could not finish signing you in. Please refresh.'); }
    } else setMessage(result.error || 'Please try again.');
  };
...
function App() {
  ...
  const [workerSession, setWorkerSession] = useState<boolean | null>(null);
  ...
  useEffect(() => subscribeToAuth((nextUser) => {
    ...
    setUser(nextSession);
    if (!nextUser) {
      setWorkerSession(false);
      return;
    }
    setWorkerSession(null);
    void isCurrentUserWorker().then((worker) => {
      setWorkerSession(worker);
      if (worker) setLocation('/worker');
    }).catch(() => setWorkerSession(false));
  }), []);
  useEffect(() => {
    if (!user) {
  ...
  if (user && workerSession === null) return <main className="flex min-h-screen items-center justify-center bg-[#b99a63] px-5 pt-24"><p className="font-mono-ui text-[10px] uppercase tracking-[.16em]">Checking your workspace…</p></main>;
  if (user && workerSession) return <WorkerPage user={user} />;
  if (location === '/worker') return <WorkerPage user={user} />;

...
function PageBackButton({ fallback = '/', label = 'Back' }: { fallback?: string; label?: string }) {
  const [, setLocation] = useLocation();
  const goBack = () => {
    if (typeof window !== 'undefined' && window.history.length > 1) window.history.back();
    else setLocation(fallback);
  };
  return <button type="button" className="new-page-back" onClick={goBack}><ArrowLeft size={15} /> {label}</button>;
}
...
function StoreHeader({ count, user, onCart }: { count: number; user: CustomerUser | null; onCart: () => void }) {
  ...
  const isActive = (href: string) => href === '/furniture' ? location.startsWith('/furniture') : location.startsWith(href);
      ...
      <header className="new-header">
        <div className="new-header__inner">
          ...
          <nav className="new-nav" aria-label="Primary navigation">
            <Link href="/furniture" className={isActive('/furniture') ? 'is-active' : ''}>Shop</Link>
            <Link href="/furniture/category/living-room" className={location === '/furniture/category/living-room' ? 'is-active' : ''}>Living room</Link>
            <Link href="/inspiration" className={isActive('/inspiration') ? 'is-active' : ''}>Journal</Link>
          </nav>
          <div className="new-header__tools">
            ...
            <Link href="/wishlist" className="new-icon-link" aria-label="Wishlist"><Heart size={18} /></Link>
...
export function RedesignedStorefront(props: StorefrontProps) {
  ...
  return <div className="new-storefront"><StoreHeader count={props.cartItems.length} user={props.user} onCart={() => setBagOpen(true)} />{props.isAdmin && <Link href="/admin" className="new-owner-link">Owner control room <ArrowUpRight size={13} /></Link>}<div className="new-announcement">{props.content.announcement}</div><Switch><Route path="/"><HomePage {...props} /></Route><Route path="/furniture"><FurniturePage {...props} /></Route><Route path="/furniture/category/:categorySlug"><CollectionPage {...props} /></Route><Route path="/furniture/:productId"><ProductPage {...props} /></Route><Route path="/wishlist"><WishlistPage {...props} /></Route><Route path="/checkout"><CheckoutPage {...props} /></Route><Route path="/account"><AccountPage {...props} /></Route><Route path="/inspiration"><InspirationPage /></Route><Route component={NotFound} /></Switch><StoreFooter /><BagDrawer items={props.cartItems} open={bagOpen} onClose={() => setBagOpen(false)} onRemove={props.onRemove} onIncrement={props.onIncrement} onDecrement={props.onDecrement} /></div>;
}
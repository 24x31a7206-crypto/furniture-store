...
import { BarChart3, Check, ChevronDown, ImagePlus, Package, Pencil, Plus, RefreshCw, Save, Search, ShieldCheck, Trash2, Upload, X, type LucideIcon } from 'lucide-react';
...
export function AdminDashboard({ user, onCatalogChange }: { user: User | null; onCatalogChange: () => Promise<void> }) {
  ...
  const refresh = async () => {
    const [catalog, allOrders] = await Promise.all([loadCatalog(), loadAllOrders()]);
    setProducts(catalog || []);
    setOrders(allOrders);
  };
